...
RETURNING department`
	...
	arr := module.FileToArr(module.DataInDir + "departments.csv")[1:]
	sizeVec := len(arr[0])
	for _, vec := range arr {
		if len(vec) == sizeVec {
			err = db.QueryRow(sqlStatement, vec[0], vec[1], vec[2], vec[3], vec[4], vec[5], vec[6], vec[7], vec[8], vec[9], vec[10], vec[11], vec[12], vec[13], vec[14], vec[15], vec[16], vec[17], vec[18]).Scan(&department)
...
RETURNING termial`
	...
	arr := module.FileToArr(module.DataInDir + "terminals.csv")[1:]
	sizeVec := len(arr[0])
	for _, vec := range arr {
...
func InsertOtborFromFile() string {
	...
	ok := true
	fmt.Println("\n\tInsertOtborFromFile...")
	arr := module.FileToArr(module.DataInDir + "otbor.csv")[1:]
	for _, vec := range arr {
...
func SelectAllDepartments() {
	...
	module.TextToFile(outText, module.DataInDir+"pg_departments.csv")
}
...
func SelectAllTerminals() {
	...
	module.TextToFile(outText, module.DataInDir+"pg_terminals.csv")
}

func MkVsyoZapros() {
	dede := module.FileToArr(module.DataInDir + "pg_departments.csv")
	deps := dede[1:]
	depsHead := "departments." + strings.Join(dede[0], ";")

	tete := module.FileToArr(module.DataInDir + "pg_terminals.csv")
	terms := tete[1:]
	...
	module.TextToFile(outText, module.DataInDir+"pg_vsyo_zapros.csv")
}
...
func otborFunc(start int, qual int) {
	...
	module.TextToFile(outText, module.DataInDir+"otbor.csv")
}

func ZaprosOtbor() {
	obobo := module.FileToArr(module.DataInDir + "otbor.csv")
	otbor := obobo[1:]
	vsyooo, _ := module.FileToVec(module.DataInDir + "pg_vsyo_zapros.csv")
	vsyo := vsyooo[1:]
	...
	module.TextToFile(outText, module.DataInDir+"pg_vsyo_zapros_vnesh_otbor.csv")
}
